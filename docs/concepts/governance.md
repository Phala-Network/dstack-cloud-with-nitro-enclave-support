...
dstack supports use on-chain smart contracts to enforce that key management policies cannot be silently altered. This is critical because:

- **Code is law:** Once a workload is deployed, its code and configuration cannot be changed without going through a visible, auditable process.
- **Observable authorization:** All changes to key access policies are recorded on-chain. Anyone can verify which workloads are authorized.
- **Covert deployer protection:** Prevents a malicious operator from secretly injecting code into a workload by updating it off-chain. If code changes, the measurements change, and on-chain re-authorization is required.
...
![Governance Workflow](../images/governance-workflow.png)
...
- **[Deploy On-chain KMS Smart Contracts](../how-to/deploy-onchain-kms.md)** — Set up contracts on testnet or mainnet
- **[Manage Governance](../how-to/manage-governance.md)** — How to create proposals and execute governance actions
- **[Security Model](security-model.md)** — Trust boundaries and security guarantees
