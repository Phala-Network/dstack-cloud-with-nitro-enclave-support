...
![Nitro Enclave Architecture](../images/nitro-enclave-architecture-v2.png)
...
The `OS_IMAGE_HASH` must be registered via [governance](governance.md) before KMS will deliver keys to this Enclave.
...
In practice, these steps are automated by deployment scripts. See [Run a Workload on AWS Nitro](../how-to/run-dstack-on-nitro.md) for the full workflow using the `deploy_host.sh` and `get_keys.sh` scripts.
...
![VSOCK Communication](../images/vsock-communication-v2.png)
...
- **[Quick Start: Deploy on AWS Nitro](../tutorials/get-started.md)** — Hands-on tutorial
- **[Run a Workload on AWS Nitro](../how-to/run-dstack-on-nitro.md)** — Detailed deployment guide
- **[Attestation Integration](attestation-integration.md)** — How attestation works on Nitro
