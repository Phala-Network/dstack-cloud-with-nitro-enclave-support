...
- GCP TDX CVM — see [Run a dstack-kms CVM on GCP](run-dstack-kms-on-gcp.md)
...
Use governance/multisig/timelock flow. See [Register Workload Measurements](register-enclave-measurement.md).
...
Template uses `dstack-util get-keys` and VSOCK proxy. Keys are returned to:

- `/var/run/dstack/keys.json` (inside enclave flow)
- host-captured file via VSOCK in helper scripts

---

## What Changes OS_IMAGE_HASH

All of these affect measured hash:

- KMS URL (baked into entrypoint)
- APP_ID (baked into entrypoint)
- `app/root_ca.pem`
- entrypoint/script content and included binaries

Use the **exact same inputs** between `--show-mrs` and real run.
...
| Aspect | GCP CVM | Nitro Enclave |
|--------|---------|---------------|
| Deployment | `dstack-cloud` | `build-eif` + `nitro-cli` |
| OS in TEE | dstack OS | AWS enclave OS |
| Guest Agent | Yes | No (`dstack-util`) |
| App format | `docker-compose` | Dockerfile/EIF |
| Disk encryption | automatic | app-managed |
| Networking | direct/gateway | host VSOCK proxy |

---

## Next Steps

- [Register Workload Measurements](register-enclave-measurement.md)
- [Run a dstack-kms on GCP](run-dstack-kms-on-gcp.md)
- [Nitro Enclave concept](../concepts/nitro-enclave.md)
